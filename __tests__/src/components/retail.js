...
import {mapStateToProps,mapDispatchToProps } from '../../../src/containers/Retail';
import ShallowRenderer from 'react-test-renderer/shallow';
import Retail from '../../../src/containers/Retail';
import configureMockStore from 'redux-mock-store';
import {fetchItems} from '../../../src/actions/item';
...
describe('Retail Test',()=>{
    ...
    beforeEach(() => {
        ...
        wrapper = mount(
            <Retail store={store} fetchItems={fetchItems}/>
        );
    });
    it('should match the snapshot of Retail',()=>{
    ...
    it('should show equal to initial mapped state', () => {
        const initialState = {
            items: ["nike","reebok"]
        };
        expect(mapStateToProps(initialState).items).toEqual(["nike","reebok"]);
    });


    it('should dispatch action of type fetch_items', () => {
        const dispatch = jest.fn();
        mapDispatchToProps(dispatch).fetchItems();
        expect(dispatch.mock.calls[0][0].type).toEqual("fetch_items");
    });
})
